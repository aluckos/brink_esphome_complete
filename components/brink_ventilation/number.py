...
BrinkU4Number = brink_ventilation_ns.class_("BrinkU4Number", number.Number, cg.Component)
BrinkU5Number = brink_ventilation_ns.class_("BrinkU5Number", number.Number, cg.Component)
...
CONFIG_SCHEMA = cv.typed_schema(
    {
        ...
        "U4": number.number_schema(BrinkU4Number).extend(
            {cv.GenerateID(BRINK_VENTILATION_ID): cv.use_id(BrinkOpenTherm)}
        ).extend(cv.COMPONENT_SCHEMA),
        "U5": number.number_schema(BrinkU5Number).extend(
            {cv.GenerateID(BRINK_VENTILATION_ID): cv.use_id(BrinkOpenTherm)}
        ).extend(cv.COMPONENT_SCHEMA),
    },
    key=CONF_TYPE,
...
async def to_code(config):
    ...
    if number_type == "VENTILATION":
        ...
        var = cg.new_Pvariable(config[CONF_ID])
        await number.register_number(var, config, min_value=0, max_value=50, step=0.5)
        cg.add(var.set_parent(parent))
        cg.add(parent.set_u4_number(var))
        # Publish initial value to 15°C (bypass atmospheric temp threshold)
        cg.add(var.publish_state(15))
    elif number_type == "U5":
        var = cg.new_Pvariable(config[CONF_ID])
        await number.register_number(var, config, min_value=0, max_value=50, step=0.5)
        cg.add(var.set_parent(parent))
        cg.add(parent.set_u5_number(var))
        # Publish initial value to 20°C (bypass indoor temp threshold)
        cg.add(var.publish_state(20))
